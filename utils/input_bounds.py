...
class Input_bound():
    ...
    def generate_bounds_single_ranges(self, split):
        ...
        if split == 2:
            ...
            param_ranges = [np.arange(split) for _ in range(self.params_per_group)]
    ...
    def float_uniform_random_generator(self, rsample, upper_bound, lower_bound = 0):
        if upper_bound > 0 and lower_bound >= 0:   
            ...
            int_sample = np.random.randint(low=int_upper_bound, high=int_lower_bound, size=1, dtype=np.int64)
            float_sample = int_sample.view(np.float64)
            ...
            raise NotImplementedError("currently we do not support ranges that cross 0")
        return float_sample
